...
@Repository
public interface OrRowRepository extends JpaRepository<OrRowEntity, OrRowId>, SuccessionsApplicableRepository {

    @Query("Select ort from OrRowEntity ort where ort.orRowId.ortStatus != 'G'")
    List<OrRowEntity> findByStatusSuccessionRelevant();

    @Query("Select ort from OrRowEntity ort where ort.orRowId.ortStatus = 'G' and ort.orRowId = ?1 and ort.outdatedAt is null order by ort.version asc")
    List<OrRowEntity> findValidById(OrRowId id);
    ...
    @Query("update OrRowEntity ort set ort.outdatedAt = :date where ort.uuid = :uuid")
    void outdate(@Param("uuid") UUID uuid, @Param("date") LocalDateTime date);

    @Modifying(flushAutomatically = true, clearAutomatically = true)
    @Query("update OrRowEntity ort set ort.orRowId.ortStatus = :ortStatus where ort.uuid = :uuid")
    ...
    @Query
    OrRowEntity getByOrRowId(OrRowId orRowId);
