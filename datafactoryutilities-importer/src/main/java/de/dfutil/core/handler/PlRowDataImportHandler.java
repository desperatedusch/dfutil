...
@Service
public class PlRowDataImportHandler implements DataImportHandler {

    private static final Logger log = LoggerFactory.getLogger(PlRowDataImportHandler.class);

    @Autowired
    private PlRowRepository jpaDao;

    public PlRowDataImportHandler(PlRowRepository jpaDao) {
        this.jpaDao = jpaDao;
    }
