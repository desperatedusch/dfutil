...
@Service
public class PlzRowDataImportHandler implements DataImportHandler {

    private static final Logger log = LoggerFactory.getLogger(PlzRowDataImportHandler.class);

    @Autowired
    private PlRowRepository jpaDao;

    public PlzRowDataImportHandler(PlRowRepository jpaDao) {
        this.jpaDao = jpaDao;
    }
